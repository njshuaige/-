...
detector = Detector()
sender = Sender(port="COM12", baudrate=9600)
...
while True:
    ...
    if current_time - last_process_time >= 0.3:
        ...
        # print("黑棋位置:", black)  # 如果不需要黑棋位置可以注释
        print("白棋位置:", white)
        ...
        if grid_instance:
            ...
            if winner != 0:
                print("胜者是：", "白棋" if winner == 1 else "黑棋")
                sent_once = True  # 禁止再发送
            else:
                # === AI 推荐下一步（始终为白棋）===
                recommended = help_move(grid_status, player=1)
